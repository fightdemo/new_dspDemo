<template>
    <div class="table">
        <div class="table-top cl">
            <h3 class="table-title">订单报表</h3>
            <div class="table-filter">
                <search placeholder="搜索订单活动ID或名称"  @search="search"></search>
                <select-data @select="selectDate"></select-data>
                <m-select :title="firstTitle" :list="activityList" :idVal="$route.params.activityID || ''" keyName="activityID" valueName="name" @change="changeActivity"></m-select>
...
export default {
    ...
    created() {
        if(this.$route.params.activityID) {
            var option = {
                ...
                secondTitle: this.$route.params.secondTitle
            }     
            this.option = option;      
            this.changeActivity(option)
    ...
    watch: {
        activityList(val) {
            val.forEach(item => {
                if(this.$route.params.activityID == item.activityID) {
