...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		bindToolbar: function() {
			var _this = this;
			// var html = $('<li class="ui-paging-toolbar"><span>跳转到</span><input type="text" class="ui-paging-count"/> 页<select class="ui-select-pagesize"></select><span> </span></li>');
			var html = $('<li class="ui-paging-toolbar"><select class="ui-select-pagesize"></select><span> </span></li>');
			var sel = $('.ui-select-pagesize', html);
			var str = '';
			for (var i = 0, l = this.settings.pageSizeList.length; i < l; i++) {
				str += '<option value="' + this.settings.pageSizeList[i] + '">'+'每页展示'+ + this.settings.pageSizeList[i] + '条/页</option>';
			};
			sel.html(str);
			...
			html.insertBefore(".js-page-last");
		}
	}
	return Paging;
});
